...
![美团外卖订餐平台 E-R 关系图](./er_diagram.webp)
...
1. **虚拟生成列 (VIRTUAL)**：建表语句中 `as (...)` 未指定 `STORED`，在 MySQL 中默认创建为 `VIRTUAL` 虚拟列。虚拟列的值不会物理存储在磁盘上，而是在每次查询读取时动态计算，节省了存储空间。
2. **`Birthdate` 的日期拼接与类型转换**：利用 `substring` 分别提取身份证号中的年份（第 7 位起 4 位）、月份（第 11 位起 2 位）和日期（第 13 位起 2 位），通过 `concat` 函数以 `-` 连接，拼接为标准 `'YYYY-MM-DD'` 格式，再通过 `date()` 函数将其隐式转换为 MySQL 的 `DATE` 日期类型。
3. **`Gender` 的性别自动映射与 `enum` 限制**：提取身份证第 17 位数字，通过对 2 取模 `% 2` 判定奇偶性，使用 `if` 函数将其映射为 `'男'` 或 `'女'`；同时列类型定义为 `enum ('男','女')`，确保了列值的有效性与空间占用最小化。
4. **命名一致性与外键约束**：字段名称与外键的参考表必须完全匹配（如 `SiteID` 作为外键关联 `Station` 表的 `SiteID`），确保数据库的参照完整性。
:::
