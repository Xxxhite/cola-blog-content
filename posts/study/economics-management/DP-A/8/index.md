...
### 8.2.1 邻接表模型 (Adjacency List)

每个结点增加一个 `ParentNodeID` 字段指向其父结点。这是最常用也是最符合直觉的存储方式。
...
### 8.2.2 路径枚举模型 (Path Enumeration)

在表中维护一个 `Ancestor`（或 `Path`）字段，存储该节点到根节点的完整路径（如 `A#A1#`）。
