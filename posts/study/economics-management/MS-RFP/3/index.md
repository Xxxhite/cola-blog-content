...
## 进阶补充：海量数据I/O与底层图形参数
*   **极速读取方案**：面对GB级别的数据文件，原生的 `read.csv()` 性能极差。推荐使用 `data.table::fread()`（C语言多线程底层实现，速度极快）或 `vroom::vroom()`（利用内存映射技术延迟加载）。
*   **底层图形参数 (par)**：R的基础图形系统由底层的 `graphics` 包控制。在进行专业排版时，常需要精确控制边缘空白：`mar` 参数控制图形内部边界（默认 `c(5, 4, 4, 2) + 0.1`），`oma` 控制外部边界，常用于在多图组合（`mfrow`）时留出总标题空间。
