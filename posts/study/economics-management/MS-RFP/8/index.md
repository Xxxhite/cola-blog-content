...
## 进阶补充：ANOVA 的线性模型本质与平方和类型
*   **ANOVA 即一般线性模型（GLM）**：在数学上，方差分析与线性回归是等价的，其本质是通过引入虚拟变量（Dummy Variables）来拟合分类自变量的线性模型（`Y = X \eta + \epsilon`）。在 R 中，`aov()` 实际上就是 `lm()` 的包装器。
*   **I 类、II 类与 III 类平方和的深层陷阱**：
