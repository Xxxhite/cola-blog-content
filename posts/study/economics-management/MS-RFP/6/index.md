...
qnorm(0.95, mean = 0, sd = 1)         # 标准正态分布，累计概率0.95的分位数

# t 分布
pt(-2, df = 10)                       # P(t < -2)
qt(0.975, df = 20)                    # 自由度20的双尾分位数
```
...
*   **极大似然估计的数值求解**：在不知道总体分布参数时，可以构建对数似然函数，然后利用R中的通用优化函数 `optim()` 或一维优化 `optimize()` 进行数值寻优，从而求得 MLE 估计值。
    ```r
